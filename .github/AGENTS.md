...
- **`integration-db`:** after parallel jobs — Supabase start, `db:reset`, declarative schema diff check, `db:lint`, `db:test:rls`, `db:check-types`, `seed`, `validate`
- **E2E:** after `build` + `integration-db` — Supabase start, `seed`, Playwright (`CI=true`, 1 worker)
...
- `validate`, RLS tests, and declarative schema diff run in the **`integration-db`** job.
