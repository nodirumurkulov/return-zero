...
- **E2E:** after parallel jobs — Supabase start, `seed`, Playwright against production build (`CI=true`, 1 worker)
...
- `validate` and RLS tests are **local-only** (seeded Supabase); run before schema/metrics PRs.
