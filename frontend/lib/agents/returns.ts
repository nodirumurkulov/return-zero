...
function createReturnsAgent(supabase: AgentSupabase, organizationId: string) {
...
The summary must be 1-2 sentences, specific, with exact numbers. The detail is structured data.`,
    tools: createReturnsTools(supabase, organizationId),
...
export async function runReturnsAgent(
  ...
  const agent = createReturnsAgent(supabase, organizationId);
