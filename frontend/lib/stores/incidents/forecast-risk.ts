...
export class ForecastRiskDetector {
  constructor(private readonly supabase: SupabaseClient) {}

  async run(opts: ForecastDetectOpts): Promise<ForecastDetectionResult> {
    const [
      ...
      this.supabase
        ...
        .eq("enabled", true),
      this.supabase.from("business_settings").select("key, value").eq("organization_id", opts.organizationId),
      this.supabase.rpc("product_daily_outflow", {
      ...
      this.supabase
        ...
        .eq("organization_id", opts.organizationId),
      this.supabase.from("products").select("id, title").eq("organization_id", opts.organizationId),
    ]);
    ...
    for (const r of outflowRows ?? []) {
      outflow.set(r.product_id as string, {
        daily: Number(r.daily_outflow ?? 0),
