...
export function formatDetectionReasonFromEvent(event: TimelineEvent): DetectionReason | null {
  ...
  if (event.event_type === "anomaly_detected" && event.description) {
    return {
      kind: "simple",
...
export function getDetectionReason(
  ...
  const anomaly = timeline.find(
    (e) => e.event_type === "anomaly_detected" && e.metadata != null,
  );
  if (anomaly) {
    const reason = formatDetectionReasonFromEvent(anomaly);
    if (reason) return reason;
  }

  const anomalyDesc = timeline.find((e) => e.event_type === "anomaly_detected");
  if (anomalyDesc?.description) {
    return {
      kind: "simple",
      summary: anomalyDesc.description,
      lines: [],
    };
  }
